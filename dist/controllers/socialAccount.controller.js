...
export const removeSocialAccount = asyncHandler(async (req, res) => {
    ...
    if (!id || !projectId)
        throw new ApiError(400, "Invalid ID or Project ID");
    ...
    if (project.userId.trim() !== req.user.id.trim()) {
        throw new ApiError(403, "Unauthorized: This project does not belong to you");
    }
    ...
    res
        .status(200)
        .json(new ApiResponse(200, account, "Social account removed successfully"));
});
export const getSocialAccountById = asyncHandler(async (req, res) => {
    if (!req.user)
        throw new ApiError(401, "Unauthorized");
    const { id, projectId } = req.params;
    if (!id || !projectId)
...
export const getSocialAccountsByProjectId = asyncHandler(async (req, res) => {
    if (!req.user)
        throw new ApiError(401, "Unauthorized");
    ...
    res
        .status(200)
        .json(new ApiResponse(200, accounts, "Social accounts retrieved successfully"));
});
